...
import { Brain, Award, RefreshCw, ChevronRight, ChevronLeft, CheckCircle2, ArrowRight, XCircle, Lock } from 'lucide-react';
import { useQuiz } from '../context/QuizContext';
import { QuizCategory } from '../types';

const QuizSection: React.FC = () => {
  // Use context instead of constants
  const { categories, userProgress, updateUserProgress } = useQuiz();
  ...
  // Check if the "Zona relax" is unlocked
  const isRelaxUnlocked = () => {
    // Get all categories EXCEPT the relax one
    const standardCategories = categories.filter(c => c.id !== 'relax');
    const totalStandard = standardCategories.length;
    
    if (totalStandard === 0) return true;

    // Count how many categories have >= 50% progress
    const completedCount = standardCategories.reduce((count, cat) => {
      const progress = userProgress[cat.id];
      if (progress && (progress.highScore / progress.totalQuestions) >= 0.5) {
        return count + 1;
      }
      return count;
    }, 0);

    // Unlock if at least 50% of categories are "completed"
    return completedCount >= Math.ceil(totalStandard / 2);
  };

  const handleCategorySelect = (category: QuizCategory) => {
    if (category.id === 'relax' && !isRelaxUnlocked()) return;
  ...
  const handleNextQuestion = () => {
    ...
    if (nextQuestion < selectedCategory.questions.length) {
      ...
      setShowScore(true);
      // Save progress
      updateUserProgress(selectedCategory.id, score + (selectedAnswer === selectedCategory.questions[currentQuestion].answer ? 1 : 0), selectedCategory.questions.length);
    }
  };
        ...
        {!selectedCategory ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 animate-fade-in">
            {categories.map((category) => {
              const isLocked = category.id === 'relax' && !isRelaxUnlocked();
              const progress = userProgress[category.id];
              const percent = progress ? Math.round((progress.highScore / progress.totalQuestions) * 100) : 0;
              ...
              return (
                <button
                  ...
                  disabled={isLocked}
                  className={`group relative overflow-hidden border rounded-2xl p-6 text-left transition-all duration-300 ${
                    isLocked 
                      ? 'bg-slate-800 border-slate-600 cursor-not-allowed' 
                      : 'bg-white/5 border-white/10 hover:bg-white/10 hover:-translate-y-1 hover:shadow-2xl hover:border-white/20'
                  }`}
                >
                  <div className={`absolute top-0 right-0 w-24 h-24 bg-gradient-to-br ${category.color} opacity-10 rounded-bl-full ${!isLocked && 'group-hover:scale-150'} transition-transform duration-500`}></div>
                  
                  {category.id === 'relax' && !isLocked && (
                    <span className="absolute top-3 right-3 bg-yellow-400 text-slate-900 text-[10px] font-black px-2 py-0.5 rounded-full z-20 animate-pulse shadow-lg">
                      NUEVO
                    </span>
                  )}

                  <div className="flex justify-between items-start mb-4">
                    <div className={`inline-flex p-3 rounded-xl shadow-lg text-white ${isLocked ? 'bg-slate-700' : `bg-gradient-to-br ${category.color}`}`}>
                      {isLocked ? <Lock size={24} /> : category.icon}
                    </div>
                    {progress && !isLocked && (
                      <div className={`text-xs font-bold px-2 py-1 rounded-full ${percent >= 50 ? 'bg-green-500/20 text-green-400' : 'bg-white/10 text-white/60'}`}>
                        {percent}%
                      </div>
                    )}
                  </div>

                  <h3 className={`text-xl font-bold mb-2 transition-colors ${isLocked ? 'text-slate-400' : 'group-hover:text-yellow-400'}`}>
                    {category.title}
                  </h3>
                  <p className={`text-sm ${isLocked ? 'text-slate-500' : 'text-blue-200'}`}>
                    {isLocked ? "Desbloquea completando el 50% de los otros módulos." : category.description}
                  </p>
                  ...
                  {isLocked && (
                    <div className="mt-4 flex items-center text-xs font-bold uppercase tracking-wider text-slate-500">
                      <Lock size={14} className="ml-1 mr-2" /> Bloqueado
                    </div>
                  )}
                </button>
              );
